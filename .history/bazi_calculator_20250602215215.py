...
def calculate_bazi_with_solar_correction(dob, birth_time, local_longitude, utc_offset):
    ...
    # For traditional day calculation: flip day at 子时 (23:00–00:59)
    if solar_dt.hour in [23, 0]:
        # Advance by one day for day pillar calculation only
        solar_dt_for_day = solar_dt + dt.timedelta(days=1)
    else:
        solar_dt_for_day = solar_dt

    # Calculate four pillars
    # Use solar_dt for year, month, hour; use solar_dt_for_day for day pillar
    Y, M, _, H = four_pillars(solar_dt, int(utc_offset))
    _, _, D, _ = four_pillars(solar_dt_for_day, int(utc_offset))
