...
def display_pillars_table(result):
    ...
    # Map to only Weak/Strong
    if verdict not in ["身强 (Strong)", "身弱 (Weak)"]:
        if verdict.lower().startswith("strong") or verdict.startswith("身强"):
            verdict = "身强 (Strong)"
        else:
            verdict = "身弱 (Weak)"
    # Choose color
    strength_color = "#fab74b" if "强" in verdict or "Strong" in verdict else "#44c4fa"
    ...
    table += f"""
        <tr>
            <td colspan="4" class="strength-row" style="color:{strength_color};">
