...
# Earthly Branch hidden stems dictionary
BRANCH_HIDDEN = {
    "子": ["癸"],
    "丑": ["己", "癸", "辛"],
    "寅": ["甲", "丙", "戊"],
...
def get_hidden_stems(branch):
    """Return list of hidden heavenly stems for one branch."""
    return BRANCH_HIDDEN.get(branch, [])

def get_pillar_hidden_stems(year_branch, month_branch, day_branch, hour_branch):
    """Return a list of hidden stem lists for the four pillars."""
    return [
...
def calculate_bazi_with_solar_correction(dob, birth_time, local_longitude, utc_offset):
    ...
    if solar_dt.hour == 23:
        # Advance by one day for both day and hour pillar calculation
        solar_dt_bazi = solar_dt + dt.timedelta(days=1)
    else:
        solar_dt_bazi = solar_dt

    # Calculate four pillars
    # Use solar_dt for year, month; use solar_dt_bazi for day, hour
