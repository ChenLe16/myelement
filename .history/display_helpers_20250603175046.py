...
def display_element_star_meter(result):
    ...
    element_colors = {
        "Wood": "#58a862",       # green
        "Fire": "#f25f3a",       # red/orange
        "Earth": "#c1915b",      # brown
        "Metal": "#d1b24a",      # gold
        "Water": "#378fcf",      # blue
    }

    # Smaller, color-matched stars and better spacing
    def star_meter(score, color="#ffd700"):
        n_full = int(score)
        n_half = 1 if score - n_full >= 0.5 else 0
        # Slightly smaller star and color
        star_str = f"<span style='color:{color}; font-size:1.18em; vertical-align:middle;'>"
        star_str += "★" * n_full
        if n_half:
            star_str += "☆"
        star_str += "</span>"
    ...
    # CSS for tighter, cleaner table with bigger text
    st.markdown("""
    <style>
    .star-meter-table th, .star-meter-table td {
    ...
    .star-meter-table {
        ...
        margin-right: auto;
        border-radius: 11px;
        box-shadow: 0 1px 10px #e4eae9;
        background: #fafcfb;
    }
    .star-meter-table tr { transition: background 0.18s; }
    .star-meter-table tr:hover { background: #eef6f7 !important; }
    </style>
    """, unsafe_allow_html=True)

    table = "<table class='star-meter-table'>"
    table += "<tr><th style='font-size:1.16em;'>Element</th><th style='font-size:1.16em;'>Star Meter</th></tr>"
    for elem, val in element_strengths.items():
        ...
        table += f"<tr style='background-color:#fafcfa;'><td>{label}</td><td>{stars}</td></tr>"
    table += "</table>"
    st.markdown(table, unsafe_allow_html=True)