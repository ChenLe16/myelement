    ...
    try:
        ...
        if not location:
            ...
            if not timezone_str:
                ...
                # Add this Day Master strength summary card
                strength = result.get("strength")
                if strength:
                    st.markdown(
                        f"""
                        <div style='
                            background: linear-gradient(90deg, #3d4450 0%, #282f39 100%);
                            color: #ffe179;
                            border-radius: 9px;
                            padding: 12px 18px;
                            ...
                            font-weight: 600;
                            text-align: center;
                            box-shadow: 0 1px 9px #1e232b;
                        '>
                            Day Master Strength: <span style="color:#fa9c3b;">{strength}</span>
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )
